...
public abstract class Request {
  ...
  @NonNull
  protected abstract MappingBuilder createMappingBuilder(Path p);

  private MappingBuilder addHeaderParams(MappingBuilder request) {
    if (headersMap != null) {
      for (Map.Entry<String, String> queryParam : headersMap.entrySet()) {
        addHeaderParam(request, queryParam);
      }
    }
    return request;
  }

  private void addHeaderParam(MappingBuilder request, Map.Entry<String, String> queryParam) {
    request.withHeader(queryParam.getKey(), equalTo(queryParam.getValue()));
  }

  private MappingBuilder addQueryParams(MappingBuilder request) {
    if (queryMap != null) {
      for (Map.Entry<String, String> queryParam : queryMap.entrySet()) {
        addQueryParam(request, queryParam);
      }
    }
    return request;
  }

  private void addQueryParam(MappingBuilder request, Map.Entry<String, String> queryParam) {
    request.withQueryParam(queryParam.getKey(), equalTo(queryParam.getValue()));
  }
